...
### AD-20 — Google OAuth security debt is explicit [ADOPTED]

- **Binds:** FR-2, FR-3, NFR-10, Story 1.6, Story 1.7
- **Prevents:** a second OAuth owner, automatic email-account merge and security claims not proven by the pinned runtime.
- **Rule:** Better Auth remains the only owner of Google authorization start, callback, authorization-code exchange, `state`/PKCE lifecycle, account/user writes and session cookies. Nest may provide only a fixed-provider start facade and mounts the official callback handler; it must not create an OAuth callback, code exchange, token/session/state/nonce store or alternate identity guard. Story 1.6 uses Better Auth `1.6.23` database-backed signed-cookie `state` and PKCE S256 with exact `PUBLIC_ORIGIN` and `${PUBLIC_ORIGIN}/api/auth/callback/google`; trusted origins/callbacks are exact and never wildcarded. `account.accountLinking.disableImplicitLinking: true` is mandatory and Google is not trusted for automatic linking. The runtime does not prove atomic state consumption, authorization-request OIDC `nonce`, or authorization-code callback ID-token signature/JWKS/issuer/audience/expiry/nonce validation. These are deferred security hardening until Better Auth upstream exposes a source-proven public capability; they must not be represented as achieved. Story 1.7 exclusively owns explicit Google linking after verified-email fresh re-authentication and confirmation.
