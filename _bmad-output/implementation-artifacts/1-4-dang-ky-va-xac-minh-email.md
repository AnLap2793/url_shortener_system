...
- **Replay semantics:** Better Auth 1.6.23 dùng signed JWT email-verification token và không lưu/consume nó trong `verification` table. Token được phép mở lại trong thời hạn: valid first use chuyển `emailVerified:false→true`; replay hợp lệ khi account đã verified là no-op success, không mutation/session/delivery thứ hai. Product chấp nhận semantics này; không tạo token store song song.
