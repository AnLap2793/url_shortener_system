...
function violates(rule, dependency, area, file) {
  ...
  if (area === "apps/worker" && dependency === "packages/db") {
    return Boolean(file) && !file.endsWith("apps/worker/src/worker-composition-root.ts");
  }
...
export async function violations(root) {
  ...
  for (const [area, rule] of Object.entries(rules)) {
    ...
    for (const file of await filesUnder(join(root, area, "src"))) {
      ...
      for (const pattern of rule.forbiddenContent ?? []) {
        if (pattern.test(source)) found.push(`${relative(root, file)} contains forbidden lifecycle call ${pattern}`);
      }
